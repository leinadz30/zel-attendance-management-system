...
export class SchoolsService implements IServices {
  ...
  getByOrgCode(orgSchoolCode: string): Observable<ApiResponse<Schools>> {
    return this.http.get<any>(environment.apiBaseUrl + this.appconfig.config.apiEndPoints.schools.getByOrgCode + orgSchoolCode)
    .pipe(
      tap(_ => this.log('schools')),
      catchError(this.handleError('schools', []))
    );
