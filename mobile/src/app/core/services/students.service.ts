...
export class StudentsService implements IServices {
  ...
  getByOrgStudentId(studentCode: string): Observable<ApiResponse<Students>> {
    return this.http.get<any>(environment.apiBaseUrl + this.appconfig.config.apiEndPoints.students.getByOrgStudentId + studentCode)
    .pipe(
      tap(_ => this.log('students')),
      catchError(this.handleError('students', []))
    );
