...
import { OpsStrandsTableColumn } from 'src/app/shared/utility/table';
...
@Component({
  selector: 'app-ops-strands',
  templateUrl: './ops-strands.component.html',
  styleUrls: ['./ops-strands.component.scss'],
  host: {
    class: "page-component"
  }
})
export class OpsStrandsComponent {
  currentOperatorCode:string;
  error:string;
  dataSource = new MatTableDataSource<OpsStrandsTableColumn>();
  columnDefs = [];
  displayedColumns = [];
  isLoading = false;
  isProcessing = false;
  ...
  controlMenuItemSelected(type: "edit" | "delete", data: OpsStrandsTableColumn) {
    console.log(type, data);
    if(type === "edit") {
      const dialogRef = this.dialog.open(OpsStrandFormComponent, {
        maxWidth: '720px',
        width: '720px',
        disableClose: true,
